...
data. The variables our group decided to include were `UNIQFIREID`,
...
geospatial and temporal dimensions. One significatn challenge we faced
was the lack of negative data. Our dataset contained a list of places
...
@fig-modelTable displays statistics for each model in the selection
process.
...
: Model metrics for random forest after hyperparameter tuning
{#tbl-model-metrics}
...
### Initial Planning (Arch GIS feasability and problems)
Initially, it was thought that using a no-code dashboard system would allow the team
to focus on the conent. To this end, we evaluated ArcGIS Online. While easy to use for ad hoc analysis,
ArcGIS struggled to efficiently render map layers due to the size of the dataset.
 Given these performance limitations, we decided use a code based, dashboarding 
 framework. We evaluated Dash, Streamlit, and Shiny.

### UI/UX Design (what we wanted it to look like, what features we wanted)
...
### Software Stack (language, libraries, deployment systems...)

When selecting the dashboard framework, the two major options were Python Dash, and R Shiny.
We selected Dash because the majority of the ETL code was already written in python.
...
We initially designed the application to query data directly from disk each time the map inputs changed. 
...
Because of this, it was decided to plot aggregated data instead of raw data, this reduced our data size to 1/100th of what it was
 By loading the necessary data into memory (approximately 1.2 GB) during the  application's initialization phase,
  we were able to use in-memory operations to manage our data. This had the added benefit on removing the need for making the full dataset
  available to the app. The necessary parquet files are small enough to be packaged directly into the docker image.

The only major issue we had was with dash-leaflet: the hexagon tile layer refused to update when the date input was changed,
