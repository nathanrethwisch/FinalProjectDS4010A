...
-   Chose dash because data was large and required custom real-time
    querying, needed efficient real-time map generation and rendering\
...
Initially, ArcGIS’s online dashboarding tools were considered for
visualizing the model outputs due to their strong integration with
geospatial data. However, the platform struggled to efficiently render
the hexagonal map layers with daily predictions, resulting in extremely
slow loading times. Given these performance limitations, the decision
was made to shift to Python’s Dash library.

Dash offered the flexibility and scalability needed to manage large
datasets and provided robust support for adding interactive map layers.
This made it a more suitable choice for building a responsive and
user-friendly dashboard capable of handling the project's data volume
and complexity.
...
- disk vs in-memory
- reducing duplicated memory for multiple clients
- speeding map generation
