...
In order to create cleaned tables, it was necessary first to mirror the needed data locally. A python script 
was written which parses the s3 bucket [manifest](https://noaa-ghcn-pds.s3.amazonaws.com/) and downloads all 
`daily` files with the `csv.gz` suffix for years after $1999$. The script also downloads a copy of the file 
`stations.txt`, the raw source for `stations`
